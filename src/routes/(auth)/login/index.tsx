...
export const useLoginUserAction = routeAction$((data, {cookie, redirect}) => {
  ...
  if(email === 'admin@admin.com' && password === '123456') {
    cookie.set('jwt', 'example_token', {secure: true, path: '/'})
    redirect(302, '/');
    return {
      success: true,
      token: 'example_token'
  ...
  password: z.string().min(6, 'caracteres minimos')
}));
...
export default component$(() => {
    ...
    return (
        <Form action={action} class="login-form mt-5">
            ...
            <p>
              {action.value?.success && (<code>Autenticado: Token: {action.value.token}</code>)}
            </p>
